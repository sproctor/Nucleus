#!/bin/bash
# Compiles NucleusNotificationBridge.m into per-architecture dylibs (arm64 + x86_64).
# Requires macOS 10.14+ (UserNotifications framework).
#
...
rm -f "$CACHE_BASE/darwin-aarch64/libnucleus_notification.dylib" \
      "$CACHE_BASE/darwin-x64/libnucleus_notification.dylib" 2>/dev/null || true
