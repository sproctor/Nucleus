#!/bin/bash
# Compiles nucleus_autolaunch_linux.c into a shared library for the current architecture.
...
SRC="$SCRIPT_DIR/nucleus_autolaunch_linux.c"
...
gcc -shared -fPIC \
    ...
    -Wl,-s \
    -lpthread \
    -o "$OUT_DIR/libnucleus_autolaunch_linux.so" \
    "$SRC" \
    $GIO_LIBS

echo "Built Linux shared library:"
ls -lh "$OUT_DIR/libnucleus_autolaunch_linux.so"
...
CACHED="$CACHE_BASE/$RESOURCE_ARCH/libnucleus_autolaunch_linux.so"
if [ -f "$CACHED" ]; then
    rm -f "$CACHED"
