...
| Badge (Windows) | `io.github.kdroidfilter:nucleus.badge-windows` | Windows Badge Notifications API — numeric counts and status glyphs on taskbar/Start tile via JNI (WinRT) |
...
Libraries that use JNI (`decorated-window`, `darkmode-detector`, `system-color`, `energy-manager`, `native-ssl`, `notification-macos`, `notification-windows`, `notification-linux`, `badge-windows`, `launcher-linux`) require `-keep` rules for their native bridge classes — these are handled by the plugin automatically.
