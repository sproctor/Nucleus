...
internal class InstallTypeDetector(
    ...
    fun detect(): ExecutableType =
        when (env.platform) {
            ...
            Platform.MacOS -> ExecutableType.ZIP
     ...
     */
    private fun packageType(): ExecutableType? {
        val markerFile =
        ...
        return when (env.readText(markerFile)?.trim()?.lowercase()?.removePrefix(".")) {
            "deb" -> ExecutableType.DEB
            "rpm" -> ExecutableType.RPM
    ...
    private fun ExecutableType.formatId(): String? =
        when (this) {
            ExecutableType.EXE -> "exe"
            ExecutableType.NSIS -> "nsis"
            ExecutableType.NSIS_WEB -> "nsis-web"
            ExecutableType.MSI -> "msi"
            ExecutableType.PORTABLE -> "portable"
            ...
            ExecutableType.TAR -> "tar"
            ExecutableType.SEVEN_Z -> "7z"
            ExecutableType.DEV -> null
        }
