...
class InstallTypeDetectorTest {
    private class FakeEnv(
        ...
        private val legacy: ExecutableType = ExecutableType.DEV,
    ) : InstallEnvironment {
        ...
        override fun legacyType(): ExecutableType = legacy
    }
    ...
    fun `linux APPIMAGE env wins over the legacy marker`() {
        val env = FakeEnv(Platform.Linux, envVars = mapOf("APPIMAGE" to "/x/App.AppImage"), legacy = ExecutableType.DEB)
        assertEquals(ExecutableType.APPIMAGE, detect(env))
        assertEquals("appimage", formatId(env))
    }

    @Test
    fun `linux SNAP env detects snap`() {
        assertEquals(ExecutableType.SNAP, detect(FakeEnv(Platform.Linux, envVars = mapOf("SNAP" to "/snap/app/1"))))
    }

    @Test
    fun `linux flatpak marker detects flatpak`() {
        val env = FakeEnv(Platform.Linux, files = mapOf("/.flatpak-info" to "[Application]\n"))
        assertEquals(ExecutableType.FLATPAK, detect(env))
    }
    ...
    fun `linux reads deb from resources package-type via java home`() {
        ...
        assertEquals(ExecutableType.DEB, detect(env))
    ...
    fun `linux reads rpm from resources package-type via launcher path`() {
        ...
        assertEquals(ExecutableType.RPM, detect(env))
    }
    ...
    fun `linux undetectable yields dev and a null format for platform fallback`() {
        val env = FakeEnv(Platform.Linux, legacy = ExecutableType.DEV)
        assertEquals(ExecutableType.DEV, detect(env))
        assertNull(formatId(env))
    }

    @Test
    fun `macos always resolves to zip with a null format`() {
        val env = FakeEnv(Platform.MacOS, legacy = ExecutableType.DMG)
        assertEquals(ExecutableType.ZIP, detect(env))
        assertNull(formatId(env))
    }

    @Test
    fun `windows defaults to the legacy marker`() {
        val env = FakeEnv(Platform.Windows, legacy = ExecutableType.NSIS)
        assertEquals(ExecutableType.NSIS, detect(env))
    ...
    fun `windows package-type msi overrides the legacy marker`() {
        val env =
            FakeEnv(
                ...
                legacy = ExecutableType.NSIS,
            )
        assertEquals(ExecutableType.MSI, detect(env))
