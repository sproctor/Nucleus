#!/bin/bash
# Compiles nucleus_taskbar_progress_macos.m into per-architecture dylibs (arm64 + x86_64).
...
SRC="$SCRIPT_DIR/nucleus_taskbar_progress_macos.m"
...
clang -arch arm64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_ARM64/libnucleus_taskbar_progress.dylib" "$SRC"
strip -x "$OUT_DIR_ARM64/libnucleus_taskbar_progress.dylib"

# Compile for x86_64
clang -arch x86_64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_X64/libnucleus_taskbar_progress.dylib" "$SRC"
strip -x "$OUT_DIR_X64/libnucleus_taskbar_progress.dylib"

echo "Built per-architecture dylibs:"
ls -lh "$OUT_DIR_ARM64/libnucleus_taskbar_progress.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_taskbar_progress.dylib"

# Clear NativeLibraryLoader cache so the new dylib is picked up at next run
CACHE_BASE="$HOME/Library/Caches/nucleus/native"
for arch in darwin-aarch64 darwin-x64; do
    CACHED="$CACHE_BASE/$arch/libnucleus_taskbar_progress.dylib"
    if [ -f "$CACHED" ]; then
        rm -f "$CACHED"
