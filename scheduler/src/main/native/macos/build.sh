#!/bin/bash
# Compiles nucleus_scheduler.m into per-architecture dylibs (arm64 + x86_64).
# Uses Foundation (NSDictionary, NSCalendar, NSTask) and ServiceManagement
# (SMJobCopyDictionary) for type-safe plist generation and job state queries.
#
...
SRC="$SCRIPT_DIR/nucleus_scheduler.m"
...
clang -arch arm64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_ARM64/libnucleus_scheduler.dylib" "$SRC"
strip -x "$OUT_DIR_ARM64/libnucleus_scheduler.dylib"

# Compile for x86_64
clang -arch x86_64 "${COMMON_FLAGS[@]}" \
    -o "$OUT_DIR_X64/libnucleus_scheduler.dylib" "$SRC"
strip -x "$OUT_DIR_X64/libnucleus_scheduler.dylib"

# Purge NativeLibraryLoader cache so the JVM picks up the fresh dylib
CACHE_BASE="$HOME/Library/Caches/nucleus/native"
rm -f "$CACHE_BASE/darwin-aarch64/libnucleus_scheduler.dylib" \
      "$CACHE_BASE/darwin-x64/libnucleus_scheduler.dylib" 2>/dev/null || true
echo "Cleared NativeLibraryLoader cache"

echo "Built per-architecture dylibs:"
ls -lh "$OUT_DIR_ARM64/libnucleus_scheduler.dylib"
ls -lh "$OUT_DIR_X64/libnucleus_scheduler.dylib"
