#!/bin/bash
# Compiles nucleus_scheduler_linux.c into a shared library for the current architecture.
...
SRC="$SCRIPT_DIR/nucleus_scheduler_linux.c"
...
gcc -shared -fPIC \
    ...
    -Wl,--gc-sections \
    -Wl,-s \
    -o "$OUT_DIR/libnucleus_scheduler_linux.so" \
    "$SRC" \
    $GIO_LIBS

echo "Built Linux shared library:"
ls -lh "$OUT_DIR/libnucleus_scheduler_linux.so"
...
CACHED="$CACHE_BASE/$RESOURCE_ARCH/libnucleus_scheduler_linux.so"
if [ -f "$CACHED" ]; then
    rm -f "$CACHED"
